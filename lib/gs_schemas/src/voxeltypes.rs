 ...
 *
 * `[ registry id (32 bits) | for future use | transparency_mode (2b) | solid_side (6b) | shape (6b) ]`
 */
#[derive(Copy, Clone, Default, Ord, PartialOrd, Eq, PartialEq)]
pub struct BlockId(u64);

impl BlockId {
    pub fn from_bits(registry_id: u32, shape_id: u8, solid_sides: u8, transparency_mode: u8) -> Self {
        Self(
            (registry_id as u64) << 3
                | (shape_id & 0b111111) as u64
                | ((solid_sides & 0b111111) as u64) << 6
                | ((transparency_mode & 0b11) as u64) << 12,
        )
    }
    ...
    pub fn transparency_mode_bits(self) -> u8 {
...
impl Debug for BlockId {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "BlockId(reg=0x{:08X}, shape={}, solid_sides={}, transparency_mode={})",
            self.registry_id_bits(),
            self.shape_id_bits(),
            self.solid_sides_bits(),
            self.transparency_mode_bits()
        )
    }
}
